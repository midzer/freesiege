...
GameScreen::GameScreen(const SpriteCollection *spr_coll,const CombinaisonCollection *cmb_coll,const std::string &ttf_path,SDL_Renderer* sdlRenderer,Background *background,MusicCollection *music_coll,GAMEMODE mode,PLAYERMODE modep1,PLAYERMODE modep2) {
...
void GameScreen::display_game(SDL_Renderer *sdlRenderer, SDL_Window *sdlWindow) {
	...
	Sprite pause_sprite(sdlRenderer,pause_surf);
	...
	while (!quit_game) {
		...
		while (!quit && !quit_game) {
			...
			background->draw();
			life_bars.first->draw(sdlRenderer);
			life_bars.second->draw(sdlRenderer);
			...
			foreground->draw();
			if(paused) {
				fill_rect(sdlRenderer,0,0,SCREEN_W,SCREEN_H,0,0,0,0.7);
...
void GameScreen::show_final_screen(SDL_Renderer *sdlRenderer, SDL_Window *sdlWindow) {
	...
	Sprite perfect_sprite(sdlRenderer,perfect_surf);
