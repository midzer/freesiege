...
Ram::Ram(const SpriteCollection *spr_coll,PLAYER player): Unit(player) {
	w=W;
	h=H;
	...
	anim_ram=spr_coll->get_anim_cycle_iterator("ram_walk",0.22);
	anim_ram_fight=spr_coll->get_anim_cycle_iterator("ram_fight",0.22);
	bit_ram_dead=spr_coll->get_sprite("ram_unit_cadaver");
...
void Ram::post_message(MessageQueue *mess_queue) {
	switch (state) {
	case STOP:
...
void Ram::handle_message(const Message &mess,MessageQueue *mess_queue) {
	if (!dead) {
	switch (state) {
	case STOP:
	case WALK:
...
void Ram::draw() {
	...
	case STOP:
	case WALK:
		if (player==PLAYER_1) anim_ram.get_next_bitmap()->draw(x,y);
